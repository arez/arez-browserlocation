...
GWT_EXAMPLES=%w(arez.browserlocation.example.BrowserLocationExample)

desc 'Arez-BrowserLocation: Arez component for the browsers location hash'
define 'arez-browserlocation' do
  project.group = 'org.realityforge.arez.browserlocation'
  ...
  doc.
    using(:javadoc,
          :windowtitle => 'Arez BrowserLocation API Documentation',
          :linksource => true,
          :timestamp => false,
...
define 'example', :base_dir => "#{File.dirname(__FILE__)}/example" do
  ...
  compile.with project('arez-browserlocation').package(:jar),
               project('arez-browserlocation').compile.dependencies,
               :gwt_user
